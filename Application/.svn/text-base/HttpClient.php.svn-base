...
class Application_HttpClient {
	 ...
	 * @access public
	 */
	function Application_HttpClient($debug = false) {
		$this->debug = $debug;
		$this->init ();
	}
	...
	private function initCA() {
		$pathCerts = realpath ( dirname ( __FILE__ ) . "/WebMoneyCA.crt" );
		if (file_exists ( $pathCerts )) {
			curl_setopt ( $this->ch, CURLOPT_CAINFO, $pathCerts );
		}
	}
	 ...
	 * @return string data
	 */
	public function send_post_data($url, $postdata, $timeout = 15) {
	 ...
	 * @access public
	 */
	function fetch_url($url, $ip = null, $timeout = 15) {
		// set url to post to
		curl_setopt ( $this->ch, CURLOPT_URL, $url );
	...
	private function checkError(){
		if (curl_errno ( $this->ch )) {
			Log_Log::saveToDatabase("CURL, Ошибка при получении данных: " . curl_error ( $this->ch ));
			//exit();
		}
	}
